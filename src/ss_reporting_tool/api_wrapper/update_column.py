...
def update_column(cfg: Config, reports):
    ...
    def _update_column(report):
        ...
        if not report.match_column or not report.update_column:
            print(f"Report {report.name} missing match_column or update_column")
            return

        if report.match_column not in df.columns or report.update_column not in df.columns:
            print(f"Excel file must contain columns '{report.match_column}' and '{report.update_column}'")
            return

        excel_lookup = df.set_index(report.match_column)[report.update_column].to_dict()
        ...
        if report.match_column not in column_name_to_id or report.update_column not in column_name_to_id:
            print(f"Sheet must contain columns '{report.match_column}' and '{report.update_column}'")
            return

        match_col_id = column_name_to_id[report.match_column]
        update_col_id = column_name_to_id[report.update_column]
        ...
        if not updates:
            print(f"No matching rows found to update for report {report.name}.")
            return
        ...
        for i in range(0, len(updates), batch_size):
            ...
            print(f"Updated batch {i // batch_size + 1} with {len(batch)} rows for report {report.name}.")

    print("Starting column update from Excel...")
    threader(_update_column, reports, cfg.threadcount)
